                ...
                <b-navbar-nav v-if="jwt" class="ml-auto">
                    <b-nav-item-dropdown right>
                        <template slot="button-content">
                            <em>Notifications
                                <b-badge variant="danger" v-show="unread > 0">{{ unread }}</b-badge>
                            </em>
                        </template>
                        <b-dropdown-item class="text-center" :key="notification.id"
                                         v-for="notification in notifications"
                                         :to="{name: 'user', params: {id: notification.notify_by }}">{{
                            notification.description }}
                        </b-dropdown-item>
                        <hr>
                        <b-dropdown-item class="text-center" :to="{name: 'notifications'}">All notifications
                        </b-dropdown-item>
                    </b-nav-item-dropdown>
                    <b-nav-item-dropdown right>
                        <template slot="button-content">
