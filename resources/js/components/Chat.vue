    ...
    <div v-if="jwt">
        <div v-show="display" id="chat">
            <b-row no-gutters>
                <div class="col">
                    <div class="matches">
                        <h3 class="text-center">Your matchesdffdssdfs</h3>
                        <b-button block size="sm" @click="setRoom(match)" :key="match.id" v-for="match in matches">
                            {{ match.user1_id !== current ? match.user1.username : match.user2.username }}
                        </b-button>
                    </div>
                </div>
                <div class="col">
