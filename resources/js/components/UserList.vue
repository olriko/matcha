<template>
    <div class="row">
        <b-col sm="12" md="6" lg="4">
            <label>
                <input v-on:click="sortBy('birthday')" v-bind:checked="sortType === 'birthday'" type="radio"
                       name="sort"/>
                sort by age
            </label>
        </b-col>
        <b-col sm="12" md="6" lg="4">
            <label>
                <input v-on:click="sortBy('distance')" v-bind:checked="sortType === 'distance'" type="radio"
                       name="sort"/>
                sort by distance
            </label>
        </b-col>
    ...
    export default {
        ...
        computed: {
            displayedList() {
                ...
                const to = (this.page * 30 ) - 2;
                const sign = this.sortType === 'birthday' ? -1 : 1;

                return this.list
                    .sort((a, b) => (a[this.sortType] < b[this.sortType] ? -1 : 1) * sign)
                    .map(u => {
